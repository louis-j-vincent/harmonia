...
# QUAL5 order is load-bearing: the context prior's token space and this
# module's candidate index (root*5+qual) both depend on it. Inlined 2026-07-31
# from progression_encoder (on the minimal rebuild's do-not-import list) —
# byte-identical to its definition.
...
def musx_cache_path(audio_path: Path | str) -> Path:
    ...
    """
    from harmonia_min.musx import _PROB_CACHE
...
def pool_span_features(
    ...
    """
    from harmonia_min.nnls_features import _l2, _ROLL_TO_C
...
def compute_acoustic_logp(
    ...
    logger.info("span_rescore: musx_probs cache MISS (%s) — falling back to "
                ...
                musx_cache.name)
    from harmonia_min import nnls_features as nf
    heads = nf.get_heads()
    if heads is None:
...
def load_context_scorer(corpus: str = "pooled"):
    ...
    """
    try:
        from harmonia_min.chord_context_prior import load_context_prior
