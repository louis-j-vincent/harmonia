...
from harmonia.serving.templates import HOME_TEMPLATE, LIBRARY_TEMPLATE
...
# ---------------------------------------------------------------------------
# Batch 3 (Phase 6c, read-only GET / library-page cluster). MOVED VERBATIM out
# of scripts/harmonia_server.py -- function bodies byte-identical to HEAD, the
# ONLY per-route change being @app.route -> @api.route. Every one is a pure read
# (jsonify a loaded file / render a static template / copy a job dict) with NO
# state mutation, so each is smoke-provable over HTTP as well as by move-identity.
# Each private leaf helper used by exactly one of these routes moved alongside it
# (``_load_billboard_corpus`` / ``_load_section_labels`` / ``_audio_path_for_chart``);
# their deps are all extracted leaves (config / state / loaders / templates /
# runtime). The name="" blueprint keeps the bare endpoints (classic_index,
# library, api_billboard_corpus, api_section_labels_get, api_chord_snippet,
# get_annotations, api_bar_merge_candidates, api_section_merge_candidates,
# api_job), so app.url_map stays byte-identical and url_for() still resolves.
#
# This pairs the GET counterparts of two routes whose POST already lived here:
# ``/api/section-labels/<filename>`` (POST api_section_labels_save) and
# ``/api/annotations/<filename>`` (POST post_annotations).
#
# LEFT in the server this round (deliberately not this batch): the whole
# ``/api/bar1-offset/<slug>`` GET/POST pair (offset cluster — kept together for
# the mutating/offset batch, 6d), and ``/api/job``'s writer siblings.
# ---------------------------------------------------------------------------
...
# _BILLBOARD_CORPUS_FILES lives in harmonia.serving.config; _load_billboard_corpus
# (its sole consumer, moved here alongside its route) reads it from there.
...
@api.route("/api/billboard-corpus")
...
@api.route("/api/section-labels/<filename>", methods=["GET"])
...
@api.route("/api/chord-snippet/<filename>", methods=["GET"])
...
@api.route("/api/section-merge-candidates/<filename>")
...
# ---------------------------------------------------------------------------
# Batch 4 (Phase 6c, cleanly-movable mutating / POST cluster). MOVED VERBATIM
# out of scripts/harmonia_server.py -- function bodies byte-identical to HEAD,
# the ONLY per-route change being @app.route -> @api.route. These mutate state /
# reach the network, so they are proven by move-identity + url_map identity
# (bare endpoints kept via name="") plus a validation-path smoke, not a full
# byte-diff over HTTP. Two groups:
#
#   * Pure-network POSTs (no server-owned helper): /api/yt-search
#     (api_yt_search, lazy yt_dlp), /api/tab-search (api_tab_search) and
#     /api/tab-fetch (api_tab_fetch) — both lazy harmonia.tab_fetcher.
#     _render_tab_page (tab-fetch's sole helper, a pure HTML-string leaf) moved
#     alongside it.
#   * Offset / beat writers whose helpers are pure leaves: GET+POST
#     /api/bar1-offset/<slug> (api_bar1_offset_get / api_bar1_offset_save) —
#     _bar1_offset_bounds moved to state.py this round, _load_bar1_offsets /
#     _save_bar1_offset already there; and POST /api/reinfer-from-beats/<song>
#     (api_reinfer_from_beats) + POST /api/beat-0-shift/<song> (api_beat_0_shift),
#     both self-contained (lookup_slug + AUDIO_DIR + lazy librosa/numpy, no
#     server-owned helper or persisted-offset store touched).
#
# PRE-EXISTING BUG carried verbatim (NOT introduced, NOT fixed here — fixing it
# would be a behavior change out of scope for a byte-identical move):
# api_reinfer_from_beats references a bare ``np`` (np.array, lines below) but
# never imports numpy in-body, and there is NO module-level ``np`` in either the
# old server module or this one. So its happy path (audio present) raises
# NameError -> caught by its own ``except Exception`` -> HTTP 500. The no-audio
# path (404) is reached before that line, which is why the move's smoke is
# identical before/after. A future fix is a one-line ``import numpy as np`` in
# the body; deliberately NOT added here to preserve behavior. Flagged for 6d.
#
# LEFT in the server (heavy, DEFERRED to the final batch per the 6d brief):
# /api/reinfer and /api/analyze (server-owned helpers + full pipeline imports).
# _apply_bar1_offset_to_payload also stays server-owned (render.py reaches it via
# the documented lazy back-import), so it is NOT a pure leaf and did not move.
# ---------------------------------------------------------------------------


@api.route("/api/yt-search", methods=["POST"])
def api_yt_search():
...
@api.route("/api/tab-search", methods=["POST"])
def api_tab_search():
...
@api.route("/api/tab-fetch", methods=["POST"])
def api_tab_fetch():
...
@api.route("/api/bar1-offset/<slug>", methods=["GET"])
...
@api.route("/api/bar1-offset/<slug>", methods=["POST"])
...
@api.route("/api/reinfer-from-beats/<song>", methods=["POST"])
...
@api.route("/api/beat-0-shift/<song>", methods=["POST"])
