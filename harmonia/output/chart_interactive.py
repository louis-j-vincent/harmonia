...
<style>
  ...
  * { box-sizing:border-box; }
  ...
  /* motif group wrapper — the neon outline goes on the group, not individual chords */
  .motif-group { display:inline-flex; align-items:baseline; gap:inherit;
    border-radius:8px; padding:1px 3px; transition:box-shadow .2s, transform .2s; }
  body[data-motif-style="full"].motif-active .motif-group {
    outline:1.5px solid var(--chip-color); outline-offset:2px;
    box-shadow:0 0 10px var(--chip-color), inset 0 0 8px color-mix(in srgb,var(--chip-color) 10%,transparent); }
  ...
  .motif-group.motif-hi {
    transform:scale(1.06); z-index:4;
    box-shadow:0 0 20px var(--chip-color,var(--mc-glow)),
               0 0 40px color-mix(in srgb,var(--chip-color,#00c8ff) 40%,transparent) !important; }
  /* shared chip base — individual chords inside groups get colour treatment */
  .chord.motif-chip { border-radius:4px; padding:0 2px;
    transition:color .2s; }
...
function renderAutoMotifs() {
  ...
  // Clear existing neon groups and chips
  document.querySelectorAll('.motif-group').forEach(g => {
    // unwrap: move children back before the group, then remove
  ...
  // Build contiguous runs: each run = consecutive chord indices sharing same name+pos
  // and having len >= 2 (single-chord annotations are not real motifs)
  const runs = [];
  let curRun = null;
  P.chords.forEach((_, i) => {
    const a = ann[i];
    const valid = a && a.len >= 2;
    const key = valid ? (a.name + ':' + a.pos) : null;
    if (valid && curRun && curRun.key === key && curRun.end === i - 1) {
      curRun.indices.push(i);
      curRun.end = i;
    } else {
      if (curRun) runs.push(curRun);
      curRun = valid ? {key, name: a.name, color: a.color, indices: [i], end: i} : null;
    }
  });
  if (curRun) runs.push(curRun);

  // For each run: wrap all its chord spans in a .motif-group container
  runs.forEach(run => {
    const els = run.indices.map(i => document.getElementById('chord-' + i)).filter(Boolean);
    if (els.length === 0) return;

    // All els should be siblings in the same .chords container
    const parent = els[0].parentNode;
    const group = document.createElement('span');
    group.className = 'motif-group';
    group.dataset.motifName = run.name;
    group.style.setProperty('--chip-color', run.color);
    group.style.setProperty('--mc-bg',      run.color + '22');
    ...
    // Insert group before first element, then move all els into it
...
// Hover-highlight all groups with the same motif name
document.addEventListener('mouseover', e => {
  const group = e.target.closest('.motif-group');
  if (!group) return;
  const name = group.dataset.motifName;
  document.querySelectorAll('.motif-group[data-motif-name="' + name + '"]').forEach(g => g.classList.add('motif-hi'));
});
