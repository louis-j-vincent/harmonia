...
<body><div class="sheet">
  ...
  <div class="modal" id="chordEditModal">
    ...
    <div class="modal-panel">
      <div class="modal-handle"></div>
      <div class="opt-section">
        <div class="opt-info"><span id="ce-current"></span><span id="ce-confidence"></span></div>
      </div>
      <hr>
      <div class="opt-section" style="align-items:center">
        <div class="opt-title">Root</div>
        <div class="wheel" id="chordEditWheel" style="width:200px;height:200px;margin:6px auto;"></div>
      </div>
      <hr>
      <div class="opt-section">
        <div class="opt-title">Quality</div>
        <div class="opt-row" id="ce-quality-group"></div>
      </div>
      <hr>
      <div class="opt-row">
...
(function(){
  ...
  const QUALITIES = [
    {q:"",     label:"maj"},   {q:"-",    label:"m"},
    {q:"7",    label:"7"},     {q:"^7",   label:"maj7"},
    {q:"-7",   label:"m7"},    {q:"h7",   label:"m7b5"},
    {q:"o7",   label:"dim7"},  {q:"o",    label:"dim"},
    {q:"6",    label:"6"},     {q:"sus4", label:"sus4"},
    {q:"+",    label:"aug"},
  ];
  ...
  // ── Root rotor for the chord editor — a second, independent instance of
  // the same drag/detent mechanic as the transpose rotor. Simplified: no
  // tick sound (the transpose wheel's audio helpers are scoped inside its
  // own IIFE, not worth duplicating here) — just the visual pulse. ──
  const ceWheel = document.getElementById('chordEditWheel');
  const ceRing = document.createElement('div'); ceRing.className='wheel-ring';
  ceWheel.appendChild(ceRing);
  const cePointer = document.createElement('div'); cePointer.className='wheel-pointer';
  ceWheel.appendChild(cePointer);
  const ceBtns=[];
  for(let pc=0; pc<12; pc++){
    const ang=(-90+pc*30)*Math.PI/180;
    const b=document.createElement('button');
    b.type='button'; b.dataset.pc=String(pc); b.dataset.ang=String(ang);
    const lbl=document.createElement('span'); lbl.className='lbl';
    b.appendChild(lbl);
    ceRing.appendChild(b); ceBtns.push(b);
  }
  const ceHub=document.createElement('span'); ceHub.className='hub';
  ceWheel.appendChild(ceHub);

  function ceLabelFlats(){ return typeof preferFlats==='function' ? preferFlats(0) : false; }
  function ceRelabel(){
    ceBtns.forEach(b=>{ b.querySelector('.lbl').textContent = noteName(parseInt(b.dataset.pc), ceLabelFlats()); });
  }
  function cePositionButtons(){
    const size=ceWheel.getBoundingClientRect().width||200;
  ...
  let ceRot=0, ceRoot=0, ceDragging=false, ceStartAngle=0, ceStartRot=0, ceMoved=0;
  function ceSetLabels(deg){ ceBtns.forEach(b=>{ b.querySelector('.lbl').style.transform=`rotate(${-deg}deg)`; }); }
  function ceApplyRot(deg,animate){
    ceRing.classList.toggle('snap',!!animate);
    ceRing.style.transform=`rotate(${deg}deg)`;
    ceSetLabels(deg);
  }
  function cePulse(){
    cePointer.classList.remove('tick'); void cePointer.offsetWidth; cePointer.classList.add('tick');
    if(navigator.vibrate) navigator.vibrate(6);
  }
  function ceAngleAt(x,y){
    const r=ceWheel.getBoundingClientRect();
    return Math.atan2(y-(r.top+r.height/2), x-(r.left+r.width/2))*180/Math.PI;
  }
  function ceSetRoot(pc, animate){
    ceRoot=((pc%12)+12)%12;
    ceRot=-ceRoot*30;
    ceApplyRot(ceRot, animate);
    ceBtns.forEach(b=>b.setAttribute('aria-pressed', parseInt(b.dataset.pc)===ceRoot?'true':'false'));
    ceHub.textContent = noteName(ceRoot, ceLabelFlats());
    ceUpdatePreview();
  }
  ceWheel.addEventListener('pointerdown', e=>{
    ceDragging=true; ceMoved=0; ceWheel.setPointerCapture(e.pointerId);
    ceStartAngle=ceAngleAt(e.clientX,e.clientY); ceStartRot=ceRot;
    ceRing.classList.remove('snap');
  });
  ceWheel.addEventListener('pointermove', e=>{
    if(!ceDragging) return;
    let delta=ceAngleAt(e.clientX,e.clientY)-ceStartAngle;
    while(delta>180) delta-=360; while(delta<-180) delta+=360;
    ceMoved=Math.max(ceMoved,Math.abs(delta));
    ceRot=ceStartRot+delta;
    ceApplyRot(ceRot,false);
  });
  function ceEndDrag(e){
    if(!ceDragging) return; ceDragging=false;
    const tapped = ceMoved<8 ? e.target.closest('button') : null;
    const pc = tapped ? parseInt(tapped.dataset.pc) : ((Math.round(-ceRot/30)%12)+12)%12;
    cePulse();
    ceSetRoot(pc, true);
  }
  ceWheel.addEventListener('pointerup', ceEndDrag);
  ceWheel.addEventListener('pointercancel', ceEndDrag);
  window.addEventListener('resize', cePositionButtons);

  // ── quality buttons ──
  const ceQualGroup = document.getElementById('ce-quality-group');
  let ceQual = "";
  QUALITIES.forEach(({q,label})=>{
    const b=document.createElement('button');
    b.type='button'; b.textContent=label; b.dataset.q=q;
    b.addEventListener('click', ()=>{
      ceQual=q;
      ceQualGroup.querySelectorAll('button').forEach(x=>x.classList.remove('sel'));
      b.classList.add('sel');
      ceUpdatePreview();
    });
    ceQualGroup.appendChild(b);
  });

  function ceUpdatePreview(){
    const match = QUALITIES.find(x=>x.q===ceQual);
    document.getElementById('ce-current').textContent =
      noteName(ceRoot, ceLabelFlats()) + (match ? match.label : ceQual);
  }

  // ── open / save ──
  let _editingIdx = -1;
  function openChordEditor(i){
    _editingIdx = i;
    const c = P.chords[i];
    const cur = c.lv.exact.c>=c.lv.seventh.c ? c.lv.exact : (c.lv.seventh.c>=c.lv.family.c ? c.lv.seventh : c.lv.family);
    ceQual = cur.q || "";
    ceQualGroup.querySelectorAll('button').forEach(x=>x.classList.toggle('sel', x.dataset.q===ceQual));
    ceSetRoot(c.root, false);
  ...
  mergeBtn.addEventListener('click', ()=>{
    ...
    if(mergeSelectActive){
      if(!annotateActive) annotateBtn.click();
      closeAllModals();
    }
  });
  ...
  document.addEventListener('click', e=>{
    ...
    if(mergeSel.length===2){
      const [a,b] = mergeSel;
      if(confirm(`Merge ${a.label} (bars ${a.range[0]+1}-${a.range[1]+1}) with ${b.label} (bars ${b.range[0]+1}-${b.range[1]+1})?`)){
