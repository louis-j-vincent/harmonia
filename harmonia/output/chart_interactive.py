...
<style>
  ...
  * { box-sizing:border-box; }
  ...
  /* Motif segment — wraps contiguous chips within one .chords container */
  .motif-segment {
    --cc: var(--chip-color, #00c8ff);
    display:inline-flex; align-items:baseline; gap:18px;
    outline:1.5px solid var(--cc); outline-offset:3px;
    border-radius:9px; padding:0 2px;
    transition:box-shadow .2s, filter .2s;
  }
  body[data-motif-style="full"].motif-active .motif-segment {
    box-shadow:0 0 10px var(--cc), inset 0 0 8px color-mix(in srgb,var(--cc) 8%,transparent);
  ...
  .motif-segment.motif-hi {
    ...
    box-shadow:0 0 20px var(--cc), 0 0 40px color-mix(in srgb,var(--cc) 40%,transparent) !important;
  }
  /* chord text colour inside segments */
  ...
  /* keep jazz marker visible inside segment */
...
function renderAutoMotifs() {
  ...
  // Group chord indices by (runId, chords-container) so we wrap contiguous
  // chords that share a parent — one .motif-segment per such group.
  // Key: runId + '|' + chords-container identity (we use its bar dataset).
  const segGroups = new Map(); // key -> {els, a, runId}
  P.chords.forEach((_, i) => {
    const a = ann[i];
    if (!a || a.len < 2) return;
    ...
    // identify parent by its position in the DOM (use a transient id)
    if (!parent._motifSegId) parent._motifSegId = ++runCounter;
    const key = rid + '|' + parent._motifSegId;
    if (!segGroups.has(key)) segGroups.set(key, {els: [], a, rid});
    segGroups.get(key).els.push(el);
  });
  // clean up transient ids
  document.querySelectorAll('.chords').forEach(p => delete p._motifSegId);

  // Inject one .motif-segment wrapper per group
