"""Les sections viennent d'un modèle pré-entraîné.
...
def segments(audio: str | Path, *, force: bool = False) -> list[dict]:
    ...
    if not force:
        try:
            d = cache.load_json("songformer", audio)
        except ValueError as exc:
            logger.warning("songformer : cache illisible pour %s (%s)",
                           audio.name, exc)
            d = None
        if d is not None:
    ...
    cache.save_json("songformer", audio, {"taille": taille, "segments": segs})
    return segs
...
def _dans_un_enfant(audio: Path) -> list[dict]:
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        r = subprocess.run(
            [sys.executable, "-m", "harmonia.sections.songformer",
             str(audio), str(out)],
...
if __name__ == "__main__":       # `python -m harmonia.sections.songformer AUDIO OUT`
    _enfant()
