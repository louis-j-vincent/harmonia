"""Beat This! beats + downbeats. The ONLY beat tracker.
...
Results are disk-cached via `harmonia.cache` (kind "beats", stem-keyed — same
rationale as the musx cache: fresh downloads of the same video get new
...
#: Bridge for scripts that still read the folder directly (e.g. to `ls` or
#: `rm` cached beat files) — the real convention lives in `harmonia.cache`.
CACHE_DIR = cache.folder("beats")
...
def track(audio_path: str | Path, *, use_cache: bool = True) -> dict:
    ...
    if use_cache:
        try:
            d = cache.load_json("beats", audio_path)
        except ValueError as exc:
            logger.warning("beats: corrupt cache for %s (%s) — recomputing",
                            audio_path.name, exc)
            d = None
