...
WHAT THIS FIRST VERSION DOES **NOT** SOLVE (rule #4 — state the remainder):
  * WITHIN-song tempo DRIFT is not modelled in the state-space: the lattice is a
    single constant tempo (brick0's fine-tuned grid). Slow drift is left to the
    upstream grid; a song whose tempo drifts beyond the grid shows up as low
    per-region confidence (which is the correct behaviour — it self-flags), not as
    a warp. Adding a slowly-drifting τ state is the next brick.
...
def align_fusion(audio, song_cfg: dict, *, features: Optional[dict] = None,
    ...
    # ── the four per-lattice-beat streams + reliability weights ─────────────────
    # harmony salience = per-beat chart-explainability (LOW in solos)
    if salience_weighting:
        salience = np.clip(b0._chart_fill(sections, cn), 0.0, None)
    else:
        salience = None
    # drum reliability (grid weight) — informational + confidence
    drum_rel = drum_track.reliability_at(beats)
    # bass sounding pc + reliability per lattice beat (span = [beat, next beat))
    bounds = np.concatenate([beats, [beats[-1] + beat_period]])
    audio_bass_pc, bass_rel = bassmod.bass_pc_series(bass_ch, bounds)
    # harmonic-rhythm flux (peak-picked chord changes) on the lattice
    flux = dbmod.harmonic_change_evidence(frames, ftimes, beats)

    # ── fused per-section emission curves + harmony-only baseline ───────────────
    ...
    for c in range(maxchor):
        for si, s in enumerate(sections):
            ...
            emit = fuse_emissions(harm, bcurve, hcurve, w_bass=w_bass, w_hr=w_hr)
            order.append((c, si, s))
            emissions.append(emit)
            emissions_harm.append(harm)

    # first-section onset (intro-skip / human seed), on the harmony baseline
    p0, start_diag = b0._first_section_start(emissions_harm[0], beats, seed_s)
    mgb = _min_gap_beats(beat_period)
    placements = viterbi_bar_pointer(order, emissions, N, p0, mgb)

    # attach harmony-only agreement + fused reliability per placement
    step_of = {}
    for i, (pl) in enumerate(placements):
        # find the tiled-order index for this placement (chorus, sec_idx)
        for k, (c, si, s) in enumerate(order):
            if c == pl.chorus and si == pl.sec_idx and s.n_beats == pl.n_beats:
                step_of[i] = k
                break
    for i, pl in enumerate(placements):
        k = step_of.get(i, 0)
        pl.harm = float(emissions_harm[k][pl.start_beat]) if np.isfinite(
            emissions_harm[k][pl.start_beat]) else 0.0
    ...
    emissions_by_step = {i: emissions[step_of.get(i, 0)] for i in range(len(placements))}
    ...
    # ── ONE global sub-beat phase refinement (v3, reused): slide the whole rigid
    #    lattice by a single offset that maximises the coverage-weighted agreement.
    #    A global phase slide (never a per-section warp), so the placements are
    #    unchanged — this is what makes the ear-approved timelines reproduce to the
    #    sub-beat, not just to the beat.
