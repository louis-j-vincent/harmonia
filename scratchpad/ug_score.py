...
def build_report(out_path: Path):
    ...
    # FRESH-decode scores are authoritative when present: the baked
    # docs/plots charts are stale (docs/postmusx_segment_loss.md) and the
    # splitter changed in c602a93, so anything measured on them describes an
    # older pipeline. Stale results survive only as an appendix.
    fresh = sorted(SCRATCH.glob("ug_score_fresh_*.json"))
    paths = fresh or sorted(SCRATCH.glob("ug_score_*.json"))
    songs = []
    for p in paths:
        try:
            ...
            pass
    # Louis 2026-07-31: Chain of Fools is OUT of the comparison base.
    quarantine = [s for s in songs if "chain_of_fools" in s["slug"]]
    songs = [s for s in songs if "chain_of_fools" not in s["slug"]]
    if not songs:
        raise SystemExit("no scores — run scratchpad/ug_rebake.py first")
    ...
    src = songs[0].get("source", "baked payload")
    L = ["# Where our charts are wrong — scored against Ultimate Guitar", "",
         f"**Chart source: {src}.**" + ("" if src != "fresh-decode" else
         "  Every song here was re-decoded with the shipped config "
         "(`nnls24` / `musx` bass+quality / `musx_redecode` segments / "
         "`function_family=True` / `beatthis` + `bestfit`), once, "
         "deterministically, writing to `scratchpad/rebake/` — never to "
         "`docs/plots/`. Earlier tables measured stale bakes and are kept "
         "only as an appendix."),
         "",
         "**Comparison base: 6 songs.** Chain of Fools is excluded (Louis, "
         "2026-07-31) — its harmony cannot time itself, so its alignment "
         "carries order but not placement. It is scored separately below as a "
         "quarantined chord-vs-no-chord stress case and enters no aggregate.",
         "",
    ...
    L += ["", "## The three answers (fresh output, 6-song base)", "",
          "### (a) Does MISSED still lead, and by how much?", "",
          "**Yes, and it is still the plurality, but its margin narrowed.** On "
          f"fresh output MISSED is {tot['MISSED']} of {n_bad} errors "
          f"({100*tot['MISSED']/max(n_bad,1):.0f}%) against ADDED at "
          f"{tot['ADDED']} ({100*tot['ADDED']/max(n_bad,1):.0f}%). On the stale "
          "bakes the same 6 songs gave MISSED 88 (69%) / ADDED 30 (24%). So "
          "the fresh pipeline fixed 27 misses and added one, and MISSED went "
          "69% → 59% of a smaller total (127 → 103).", "",
          "Two caveats stated plainly: **ROOT got worse** (5 → 9), and the "
          "MISSED total is now carried by just two songs — Close to You (24, "
          "unchanged) and Let It Be (18).", "",
          "### (b) What did the splitter fix change on Let It Be?", "",
          "**MISSED 37 → 18** on the fresh decode, and what remains is one "
          "shape: `C [Dm7] C` accounts for **12 of the 18**, every instance "
          "about 0.5 s. Before the fix Let It Be's misses were spread over many "
          "shapes; now they are a single repeated ornament.", "",
          "I could **not** isolate the splitter's contribution from the "
          "stale-bake effect without re-decoding under the old splitter, and I "
          "did not. The commit measured −8 against its own stale baseline; the "
          "rest of the −19 here is the bake being stale — a different, older "
          "decoder in which music-x-lab never ran. Treat the split as "
          "approximate.", "",
          "### (c) Top three recurring defect classes — the updated “où on se "
          "trompe”", "",
          "| # | class | n | shape |", "|---|---|---|---|",
          "| 1 | **Sub-beat passing chords we never write** | 61 | median "
          "**0.79 beats**; 74% last one beat or less, 6% are a full bar. Two "
          "songs carry it: Close to You (24, harmonically diverse) and Let It "
          "Be (18, of which 12 are the same `C [Dm7] C`). |",
          "| 2 | **Chords invented in outros and fades** | 31 | Hot N Cold's 22 "
          "ADDED are **all** between 216 s and 284 s; Let It Be's 3 are all "
          "199–208 s. Everything earlier in both songs is clean. This is an "
          "end-of-song failure, not a diffuse one. |",
          "| 3 | **Root errors, and they grew** | 9 | Up from 5 on the stale "
          "bakes; now spread over This Love (2), Close to You (2), Every Breath "
          "(2), Hot N Cold (2), Let It Be (1). Small but the only class moving "
          "the wrong way. |", "",
          "Quality errors have essentially vanished from the base set (2 of "
          "103) once cosmetic 6th/7th spellings are excluded.", "",
          "### The chord-vs-no-chord win, which is the biggest change of all",
          "",
          "On the stale bake Chain of Fools produced **57 ADDED chords, 7 of "
          "them inside the a-cappella passage**. The fresh pipeline produces "
          "**five spans**: N.C. 0–3.6 s, C7 3.6–82.6 s, **N.C. 82.6–99.6 s**, "
          "C7 99.6–161.1 s, N.C. 161.1–169.3 s.", "",
          "That middle no-chord span is the “(No music)” call-and-response, "
          "which the ASR anchors timed independently at **82.2–96.0 s** — the "
          "onsets agree to 0.4 s. The hallucination-in-silence failure that "
          "this whole report kept pointing at is **gone on the shipped "
          "pipeline**. What is left on that song is the third: we write C7 "
          "where the tab says Cm7, which Rule 1 correctly keeps as an error.",
          "",
          "It under-segments in the other direction now (79 s of one chord), "
          "but the tab agrees the song is one vamp, so that is defensible; the "
          "Cm/Cm7 alternation it flattens is cosmetic by Rule 1.", "",
