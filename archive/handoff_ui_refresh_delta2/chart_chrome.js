/* ─────────────────────────────────────────────────────────────────────────
  ...
  function chartToolbar(){
    ...
    // Léger à l'œil, 44 px à la main : la cible fait toute la hauteur, les
    // marges négatives l'empêchent de gonfler la barre. Un bouton sans fond
    // reste un bouton — c'est exactement le piège que STYLE.md interdit.
    const meta=el("button",
      `display:inline-flex;align-items:center;gap:6px;height:${SZ.control}px;`+
      `margin:-8px 0 -10px;border:none;background:none;padding:0;`+
      `font:500 13px ${UI};color:${T.faint};cursor:pointer;`);
