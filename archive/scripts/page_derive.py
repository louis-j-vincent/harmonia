...
SORTIE = REPO / "docs" / "plots" / "beats_derive.html"
...
def ensemble(s: dict) -> str:
    ...
    for t in jum:
        barres.append(f'<line x1="{x(t):.2f}" y1="0" x2="{x(t):.2f}" y2="26" '
                      f'stroke="#a4462b" stroke-width="1"/>')
...
def main() -> None:
    songs = json.loads(Path("/tmp/derive.json").read_text())
    blocs = []
    for s in songs:
        b1, b2, b3 = s["bpm"]
        blocs.append(f'''
  <section data-audio="{s["audio"]}" data-pxs="{PXS}">
    <h2>{html.escape(s["titre"][:44])}
      <span>{s["dur"]:.0f} s · BPM des trois zones :
      <b>{b1}</b> · <b>{b2}</b> · <b>{b3}</b>
      {f"· <b class=j>{s['n_jum']} doublons</b>" if s["n_jum"] else ""}</span></h2>
...
<h1>Les temps de Beat This!, sur toute la chanson</h1>
<p class="chapo">Les six morceaux que la grille rigide refuse. Fais glisser la
bande — elle suit le son.</p>
