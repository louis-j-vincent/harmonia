 ...
 *{{box-sizing:border-box}}
 ...
 /* Dans l'app, le compas vit dans une feuille (`kit.js::sheetBox`) : fond
    `card`, coins de 22 px, ombre portée. Il n'est jamais posé à même la page,
    et sa teinte de fond fait partie de son identité — l'orbe pâle est pâle
    PAR RAPPORT à ce blanc-crème. On lui rend donc son support, fil d'Ariane
    et barre d'outils compris, qui sont eux aussi dans la feuille. */
 .feuille{{background:var(--card);border-radius:22px;padding:14px 16px 16px;
  margin:6px 0 4px;box-shadow:0 8px 28px -18px rgba(50,35,20,.55)}}
...
// UN ORBE EST UN FOND CLAIR DANS LES DEUX THÈMES. `petalFill` et `keyTint`
// n'ont pas de branche sombre — ils sont pâles par construction, c'est ce qui
// fait l'identité du compas. Ce qu'on écrit DESSUS doit donc rester sombre
// même quand la page est sombre : prendre `--ink` y mettrait du crème sur du
// pâle. (Le compas de l'app a exactement ce défaut, cf. known_issues.)
const ENCRE_PETALE = "#1c1c1c";        // `LIGHT.ink`
const PALE_PETALE = "#6f6857";
...
const confColor = c => c >= .82 ? ENCRE_PETALE : c >= .66 ? "#5c4a30"
...
const encreSymbole = (c, premier) => premier ? confColor(c) : ENCRE_PETALE;
