...
<div class="note">
  ...
  <p><b>La teinte vient de la note, jamais de la confiance.</b> Elle suit le
  cercle des quintes&nbsp;— c'est la même roue que le compas, donc deux rayons
  voisins portent deux teintes voisines. Au premier cercle c'est la fondamentale
  qui la donne&nbsp;; plus bas c'est la note ajoutée, et la septième de
  <code>D-7</code> est un do, d'où le rouge de do.</p>
  <div class="cle" id="cleNotes"></div>
  <p><b>La confiance, elle, ne touche que la saturation et la clarté.</b> Sûr
  = foncé et franc, incertain = pâle&nbsp;: c'est <code>petalFill</code> /
  <code>petalEdge</code> tels quels. Deux réglages seulement ont été ajoutés
  ici, et pour une raison chacun. Au-delà de ~62&nbsp;% le fond devient trop
  sombre pour un texte d'encre, donc le symbole passe en crème&nbsp;— l'app ne
  rencontre pas le cas, ses pétales sont petits et rarement si sûrs.</p>
  <div class="cle" id="cleConf"></div>
  <p><b>Et <code>confColor</code> ne colore que le moyeu et le premier
  cercle.</b> Ce dégradé encre&nbsp;→ brun&nbsp;→ rouge veut dire une chose
  précise&nbsp;: « le modèle n'est pas sûr que ce soit cet accord-là ». Ce sens
  n'existe qu'au premier niveau. Plus bas, un 19&nbsp;% est une part entre
  extensions, pas un doute&nbsp;— l'appliquer peindrait en rouge d'alerte un
  compas qui ne doute pas, il répartit.</p>
...
const petalFill = (pc, c) =>
  `hsl(${{rootHue(pc)}} ${{Math.round(46 + c * 26)}}% ${{Math.round(84 - c * 34)}}%)`;
const petalEdge = (pc, c) =>
  `hsl(${{rootHue(pc)}} ${{Math.round(50 + c * 26)}}% ${{Math.round(60 - c * 22)}}%)`;
...
// DEUX GARDE-FOUS, parce que ce compas-ci n'est pas exactement celui de l'app.
//
// 1. `petalFill` fonce quand la confiance monte (clarté 84 - 34c). L'app s'en
//    sort parce que ses pétales sont petits et rarement très sûrs ; ici un
//    orbe à 83 % est un disque plein et foncé, sur lequel un texte sombre ne
//    se lit plus. Au-delà de ~62 % on écrit donc en crème.
const SOMBRE = c => 84 - 34 * c < 63;
const surFond = (c, sombre, clair) => SOMBRE(c) ? sombre : clair;
// 2. `confColor` veut dire UNE chose : « à quel point le modèle croit que
//    c'est CET accord-là ». Ce sens n'existe qu'au premier niveau. Plus bas,
//    les nombres sont des parts entre extensions (1 %, 19 %) — les peindre en
//    rouge ferait crier « le modèle doute » tout un compas qui ne doute pas,
//    il répartit. Sous le premier niveau, le symbole est donc simplement
//    lisible : encre sur fond clair, crème sur fond foncé.
const CREME = "#f7f2e6";
const encreSymbole = (c, premier) =>
  SOMBRE(c) ? CREME : (premier ? confColor(c) : css('--ink'));
...
function dessine(){{
  ...
  if (niv) {{
    ...
    nodes.forEach(n => {{
      ...
      svg.appendChild(sv("line", {{x1: cx, y1: cy, x2: x, y2: y,
        stroke: petalEdge(n.o.pc, n.o.p), "stroke-width": 1, "stroke-opacity": 0.4}}));
      const g = sv("g", {{class: "orbe"}});
      g.appendChild(sv("circle", {{cx: x, cy: y, r: n.pr,
        fill: petalFill(n.o.pc, n.o.p), stroke: petalEdge(n.o.pc, n.o.p),
      ...
      const pct = sv("text", {{x, y: y + n.pr * 0.60, "text-anchor": "middle",
        ...
        fill: surFond(n.o.p, "rgba(247,242,230,.82)", C.faint)}});
      pct.textContent = (n.o.p * 100).toFixed(0) + ' %';
      g.appendChild(pct);
      g.addEventListener('click', () => {{
        chemin.push(niv.cle === 'base'
          ? {{cle: 'base', root: n.o.root, type: n.o.type, lab: n.o.sym, p: n.o.p}}
  ...
  hub.appendChild(sv("circle", {{cx, cy, r: Sz * 0.12, fill: keyTint(moyPc),
  ...
  const pBase = chemin.length ? (chemin[0].p ?? 0)
    : ((D.accords[sel].base.find(b => b.root === D.accords[sel].root)
        || D.accords[sel].base[0] || {{}}).c ?? 0);
  glyphe(hub, cx, cy - Sz * 0.012, sym.root, sym.q, Sz * 0.105, confColor(pBase));
...
(function cles() {{
  ...
  for (let i = 0; i < 12; i++) {{
    ...
    d.className = 'pas';
    d.style.background = petalFill(pc, 0.55);
    d.style.border = '1.5px solid ' + petalEdge(pc, 0.55);
    d.style.color = css('--ink');
    d.textContent = N.notes[pc];
    notes.appendChild(d);
  }}
  const conf = document.getElementById('cleConf');
  [0.05, 0.25, 0.45, 0.65, 0.85].forEach(c => {{
    const d = document.createElement('div');
    d.className = 'pas';
    d.style.background = petalFill(2, c);   // une seule note : ré
    d.style.border = '1.5px solid ' + petalEdge(2, c);
    d.style.color = SOMBRE(c) ? CREME : confColor(c);
