...
eux-mêmes ne sont pas touchés** — le modèle reste ce que le pipeline a produit.
    ...
    python tools/backfill_titles.py             # dry-run, affiche la table
    python tools/backfill_titles.py --apply     # écrit le sidecar
    python tools/backfill_titles.py --apply --force   # écrase aussi ce qui
