...
def capture(stem: str) -> dict:
    ...
    On patche le module RÉEL (`harmonia.sections`), jamais le pont
    `harmonia_min.sections` (supprimé au sprint 9) : `harmonia.pipeline.
    analyze_steps` fait `from harmonia.sections import detect_sections` en
    LOCAL juste avant l'appel, donc c'est l'attribut du module `harmonia.
    sections` au moment de l'appel qui compte — même leçon que le sprint 10
    (« on patche le module réel, jamais le pont »).
    """
    from harmonia import pipeline as _pl
    from harmonia import sections as hs
...
def sub_voix(stem, edges):
    ...
    """
    from . import vocal_anchor as VA
    from . import vocal_melody as VM
