"""Est-ce que ça régresse, sur la QUALITÉ des sections ? Le banc de la piste
de recherche section-quality (déplacé de `scripts/` à `tools/sections_bench/`
au sprint 20 du refactor).

    .venv/bin/python -m tools.sections_bench.bench --quick    # les 4 morceaux chauds, ~10 s
    .venv/bin/python -m tools.sections_bench.bench            # TOUT ce que Louis a annoté
    .venv/bin/python -m tools.sections_bench.bench --save     # fige la référence actuelle

Sort en code 1 dès qu'un morceau recule de plus de TOL.

**N'est PLUS dans la chaîne `/ship`** (2026-09-14) : ce banc mesure la
qualité d'un DÉCOUPAGE de sections, pas la régression d'un chart — c'est
`tools.golden` (le rapport d'or) que `/ship` appelle désormais, sur les
44 morceaux de la bibliothèque plutôt que les 18 annotés section-par-section.
À réserver à la piste qualité-des-sections, qui reprend après la bascule prod
...
from . import order_bundle                                             # noqa: E402
